(function(){
	...
	var BLOCK = function(){
		...
		var query_types={
			order: 500,
			prefix: 'product_name_other',
			pattern_search: /^(.*?)$/,
			pattern_exact: /^\d{1,9}$/,
			template: $('#block-product_name_other'),
			json: {
			...
			init: function(input_query, block){
				...
				input.selectize({
					...
					load: function(query, callback) {
						var _query = $('#blocks').find('.block-product_name :input').text();
                        
						$.ajax({
							url: LANG+'/monitoring/other_names/search/products/?query=' + encodeURIComponent(query)+'&_query='+encodeURIComponent(_query),
							type: 'GET',
							dataType: 'json',
