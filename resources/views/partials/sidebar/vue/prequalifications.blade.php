<div class="tender-header__kicks tender-header-block" v-if="tender.prequalifications">
    <div v-bind:class="{ 'tender-header__wrap tender-header__descr tender-header__kick': true, 'toggled': toggled.prequalifications, 'loading': loading.prequalifications }">
        <button class="tender-header__descr-toggle" v-on:click="toggleRemote('prequalifications')"></button>
        <spinner size="small"></spinner>
        <div class="block-title">{{ t('tender.qualifications') }}</div>
        <div class="tender-header_info__item">
            <div class="kicks-container" v-if="tender.remote.prequalifications.prequalifications && tender.remote.prequalifications.prequalifications.length">
                <div v-for="qualification in tender.remote.prequalifications.prequalifications" v-bind:class="{ 'kick-item': true,  'positive': qualification.status == 'active', 'pending': qualification.status == 'pending', 'negative': (qualification.status != 'active' && qualification.status != 'pending') }">
