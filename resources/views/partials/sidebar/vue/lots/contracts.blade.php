<div class="tender-header__contracts tender-header-block" v-if="tender.contracts">
    <div v-bind:class="{ 'tender-header__wrap tender-header__descr tender-header__kick': true, 'toggled': toggledLots[lotId].contracts, 'loading': loadingLots[lotId].contracts }">
        <button class="tender-header__descr-toggle" v-on:click="toggleRemote('contracts', lotId)"></button>
        <spinner size="small"></spinner>
        <div class="block-title">{{ t('indicators.contract_block') }}</div>
        <div class="tender-header_info__item" v-if="lots[lotId] && lots[lotId].remote.contracts.contracts.length" v-for="contract in lots[lotId].remote.contracts.contracts">
